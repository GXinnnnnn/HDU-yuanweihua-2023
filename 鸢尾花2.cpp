...
/**
 * @brief 把不同种类的花分别转化成 1 2 3 标签
 * @param type[IN] 花的种类
 * @param label[OUT] 转化的标签
 */
void label123(char* type, char* label) {
    if (strcmp(type, "Iris-setosa") == 0) *label = '1';
    if (strcmp(type, "Iris-versicolor") == 0) *label = '2';
    if (strcmp(type, "Iris-virginica") == 0) *label = '3';
}
...
void openDataFile(const char* path) {
    ...
    for (int i = 0; i < TOTAL; i++) {
        for (int j = 0; j < N; j++) {
            if (fscanf_s(fp, "%lf,", &temp[i].value[j]) != 1) {
                perror("Error reading value");
                fclose(fp);
                return;
            }
        }
        if (fscanf_s(fp, "%19s", temp[i].type, (unsigned)_countof(temp[i].type)) != 1) {
            perror("Error reading type");
            fclose(fp);
            return;
        }
        /* 把不同种类的花分别转化成 A B C 标签 */
        label123(temp[i].type, &temp[i].label);
    }
...
char countLabel(int* count, int k, char forecastLabel) {
    int i;
    int sum1 = 0, sum2 = 0, sum3 = 0;  //分别统计距离最邻近的三类标签出现的频数
    for (i = 0; i < k; i++) {
    ...
    char maxLabel = compareLabel(sum1, sum2, sum3);
...
int main() {
    ...
    openDataFile("./data.txt");  // 打开数据文件 -> 打乱数据
